...
def _image(sha: str, blob: bytes | None = None) -> PptxImage:
    return PptxImage(
        slide_index=1,
...
class _StubDescriber:
    ...
    async def describe(self, image: PptxImage):
        if image.blob == self._decorative:
            return None
        return f"第{image.slide_index}页转写内容"
